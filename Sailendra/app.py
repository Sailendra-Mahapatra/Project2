...
def bars(year):
    ...
    for res_export in session.query(Exports.YTDValue, Exports.Period).filter(Exports.Period.like(f"{year}-12")).all():
        YTD = {}
        YTD={'YTDValue': res_export[0],
            'Period':res_export[1]}
        YTD_export_dict.append(YTD)

    YTD_import_dict = []
    for res_import in session.query(Imports.YTDValue, Imports.Period).filter(Imports.Period.like(f"{year}-12")).all():
        YTD = {}
        YTD={'YTDValue': res_import[0],
            'Period':res_import[1]}
        YTD_import_dict.append(YTD)

    product = [YTD_export_dict, YTD_import_dict]
